<#
.SYNOPSIS
Imports Entra ID user metadata into Juriba DPC.

.DESCRIPTION
This script authenticates to Microsoft Entra ID using application credentials,
retrieves user data from Microsoft Intune via Microsoft Graph,
...
An array of user properties to include when transforming Intune
user data for Juriba DPC. Defaults to a standard set of commonly useful
metadata fields.
...
.EXAMPLE
.\Juriba_Import_EntraId_Users.ps1 `
...
.\Juriba_Import_EntraId_Users.ps1 `
    ...
    -ClientId "11111111-1111-1111-1111-111111111111" `
    -ClientSecret "client-secret-value" `
    -UserProperties @("city","department","employeeId")
...
try {
    ...
    -Rows $users `
    -IncludeProperties $UserProperties     
}
catch {
...
# Push users to Juriba DPC
try {
    Add-LogEntry -Entry "Pushing users to Juriba DPC" -LogLevel Info
    ...
    Add-LogEntry -Entry "Unable to push users to Juriba DPC" -LogLevel Error
    Add-LogEntry -Entry "$_" -LogLevel Debug
}
