...
<#
.SYNOPSIS
Retrieves security-enabled groups from Microsoft Entra ID via Microsoft Graph.

.DESCRIPTION
Queries the Microsoft Graph `/groups` v1.0 endpoint and retrieves all
security-enabled Entra ID groups accessible to the supplied access token.
...
JSON strings to preserve the original structure for downstream processing,
such as importing group data into Juriba.

.PARAMETER AccessToken
A valid OAuth 2.0 bearer token for Microsoft Graph with permission
to read Entra ID group objects.
...
.EXAMPLE
$dt = Get-EntraIdGroup -AccessToken $AccessToken

Retrieves all security-enabled Entra ID groups accessible to the token.

.EXAMPLE
$dt = Get-EntraIdGroup -AccessToken $AccessToken -Verbose

Retrieves all security-enabled groups and outputs verbose paging information.

.NOTES
- Uses Microsoft Graph v1.0 `/groups` endpoint
...
#>
    ...
    begin {
        ...
        if ([string]::IsNullOrWhiteSpace($Uri)) {
            $Uri = 'https://graph.microsoft.com/v1.0/groups?$filter=securityEnabled eq true'
        }
    }
