...
LEFT JOIN [dbo].[WindowsServicingStates] WSS
    ON WSS.Build = OSD.Version00
	AND WSS.Branch = SD.OSBranch01
LEFT JOIN [dbo].[WindowsServicingLocalizedNames] WSLN
...
SET  operatingSystemArchitecture 	= 	LEFT(PD.AddressWidth00, 10)
	...
	,processorSpeed 				= 	PD.MaxClockSpeed00
...
-- Get network info
UPDATE C 
SET networkCardDescription 	= LEFT(NCD.Name00, 100)
...
SET  monitorCount 			= 1
	...
	,monitorModel 			= LEFT(DMD.MonitorType00, 256)
