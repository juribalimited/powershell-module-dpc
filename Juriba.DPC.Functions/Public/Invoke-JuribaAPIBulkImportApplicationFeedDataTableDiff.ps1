function Invoke-JuribaAPIBulkImportApplicationFeedDataTableDiff{
    ...
    .Description
    Takes a System.Data.Datatable object with the columns required for the DwAPI Application. Inserts these Applications one at a time.

    .Parameter Instance
    The URI to the Dashworks instance being examined.

    .Parameter APIKey
    The APIKey for an Application with access to the required resources.
    ...
    [System.Data.DataTable] Data table containing the fields required to insert data into the DW Application API.
    ...
    .Example
    # Get the Application feed id for the named feed.
    ...
    if (-not $ImportId)
    {
        ...
        try{
            ...
            write-error "Application feed lookup returned no results: $_"
            exit 1
        }
    }
    ...
    if ($APIVersion -eq 1)
    {
        $uri = '{0}/apiv2/imports/applications/{1}/items?fields=uniqueIdentifier,lastUpdated&order=uniqueIdentifier&limit=1000' -f $Instance,$ImportId
    }
    else{
    ...
    if ($APIVersion -eq 1)
    {
        $uri = '{0}/apiv2/imports/applications/{1}/items/$bulk' -f $Instance, $ImportId
    }
    else{
    ...
    for($i=0;$i -le 1;$i++)
    {
        ...
        foreach($Row in $dtApplicationData){
            ...
            {
                foreach($Device in $DPCDeviceAppDataTable.select("appUniqueIdentifier='$($Row.uniqueIdentifier)'"))
                {
            ...
            if ($BulkUploadObject.Count -eq $BatchSize -or $RowCount -eq $dtApplicationData.Rows.Count)
            {
                ...
                try{
                    $stopwatch.Start()
                    if ($Async -and $Method -eq 'Post' -and $false) #Not implemented for applications yet.
                    {
    ...
    foreach($row in $dvApplicationDelete)
    {
        ...
        if ($APIVersion -eq 1)
        {
            $deleteArray += "/imports/applications/{0}/items/{1}" -f $ImportId,$row.UniqueIdentifier
        }
        else{
